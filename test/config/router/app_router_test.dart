  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        ...
        RoutePaths.login,
        reason:
            'splash sends a failed profile fetch to login; the router '
            'must not fight that by redirecting elsewhere',
      );

      // The regression this test exists to catch: the redirect callback
      // re-runs on every navigation event. If it still treated "errored"
      // as "not onboarded", it would immediately bounce this login-page
      // landing to onboarding — which is exactly what the pre-fix guard
      // did, since it only excused `isLoading`, not `hasError`.
      expect(
        router.state.matchedLocation,
        isNot(RoutePaths.onboardingWelcome),
      );
    },
  );
}
