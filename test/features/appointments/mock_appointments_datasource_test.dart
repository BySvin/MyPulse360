...
void main() {
  ...
  test('every slot is disabled and flagged on-leave when leave is approved for that day', () {
    ...
    final slots = dataSource.getAvailableSlots(doctorId: MockIds.drAhmedUserId, date: leaveDay);
  ...
  test('a pending (not yet approved) leave request does not block slots', () {
  ...
  test('approved leave for a different doctor does not affect this one', () {
  ...
  test('a day outside the approved leave range is unaffected', () {
    ...
    final slots = dataSource.getAvailableSlots(doctorId: MockIds.drAhmedUserId, date: dayAfterLeave);
