...
- [ ] **Step 7: Make the isolation assertions non-vacuous**

Found by the Step 3 run. Two isolation assertions passed for the wrong reason:
"Aisha cannot see Daniel's appointments" and "Aisha cannot see Daniel's metrics,
goals or chats" both returned zero rows because **Daniel owns no such rows**,
not because RLS denied them. An assertion that would pass with RLS switched off
proves nothing — the same defect class as a test that cannot fail.

Fix by giving the other patient real data. Append to `supabase/seed.sql` (it is
idempotent; re-running it applies only the new rows):
...
Re-apply the seed with `execute_sql`, then **re-run the two previously-vacuous
isolation blocks unchanged**. They must still report `PASS` — Aisha still sees
zero of Daniel's rows — but now for the right reason.

**Prove they are no longer vacuous.** Run this counter-check, which confirms the
rows genuinely exist and are visible to their owner:

```sql
-- Daniel must see exactly the rows Aisha cannot.
begin;
select set_config('request.jwt.claims',
...
If that returns `FAIL`, the seed did not apply or a policy is too strict — either
way the isolation result is not trustworthy until it passes.

> Note the RPC behaviour test's rollback baseline changes: the appointment count
> is now **1**, not 0. Verify it returns to 1, not 0, after those blocks roll back.

```bash
git add supabase/seed.sql supabase/tests/0015_rls_isolation_test.sql
git commit -m "test(db): give the other patient real data so isolation tests bite"
```
